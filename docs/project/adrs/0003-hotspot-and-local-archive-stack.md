...
Prototype v1 uses a Pi-hosted WiFi hotspot backed by `hostapd` and `dnsmasq`, with Kiwix serving the local browseable archive. The oracle answer path uses a separate local index derived from curated plaintext and selected Kiwix extracts, plus a bounded direct `.zim` fallback over an explicit allowlist when the index misses. It does not query the Kiwix HTTP service live on every mesh request.
...
- allows a curated `.zim` fallback without making Kiwix HTTP availability part of the answer path
- adds operational surface area that must be covered by runbooks and drills
- allows Kiwix software and ZIM content to be refreshed independently of the retrieval index so long as derived artifacts are rebuilt
